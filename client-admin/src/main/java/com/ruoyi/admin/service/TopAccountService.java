...
public class TopAccountService extends ServiceImpl<TopAccountMapper, TopAccount> {
    ...
    public void refund(String transNo) {
        //查询对应的流水
        List<TopAccountTx> topAccountTxList = topAccountTxService.getByRefNo(transNo);
        topAccountTxList.forEach(topAccountTx -> {
            ...
            topAccountService.processAccount(
                    Arrays.asList(
                            AccountRequest.builder()
                                    ...
                                    .refNo(transNo)
                                    .remark(Account.TxType.WITHDRAW_REJECT.typeName)
                                    .build()
                    )
            );
        });
