...
@Slf4j
public class DecodeMessageUtils {
    ...
    private static String decodeMessage(String signedData) {
     ...
     */
    private static String decodeMessageV340(String signedData) {
        System.out.println("解密 start " + System.currentTimeMillis());
        RawTransaction rawTransaction = TransactionDecoder.decode(signedData);
        String from="";
        if (rawTransaction instanceof SignedRawTransaction) {
            try {
                from = ((SignedRawTransaction) rawTransaction).getFrom();
     ...
     *
     */
    public static boolean validateSign(String signedData)throws Exception {
        try {
            String signedAddress = decodeMessage(signedData);
            String fromAddress = decodeMessageV340(signedData);
            return fromAddress.equals(signedAddress);
        }catch (Exception e){
